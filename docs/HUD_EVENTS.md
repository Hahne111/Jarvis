...
| Skills (Phase 12) | `skill.reviewed` (Findings), `skill.tested`, `skill.installed|enabled|disabled|rolled_back|rejected`, `skill.log`; Daten via `GET /skills` | Install nur nach `permission.ask` (P3); Skill-Capabilities heißen `skill.<name>.<cap>` und erscheinen im Capability-Log wie alle anderen |
