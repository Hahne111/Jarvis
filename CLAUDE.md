...
- `apps/desktop/web/` – web-first HUD-Shell (ADR-0003): reines HTML/CSS/ES-Module ohne Build, vom Core unter `http://127.0.0.1:7870/hud/` ausgeliefert (`/` leitet dorthin); zentraler AI-Core aus `presence.changed`, Event Rail (WS `after_seq`, Bereichsfilter), Approvals, Missions, Memory, Kill Switch, Latenz-Kacheln, Coding-Modus (Projektbaum, Editor = Monaco **nur lokal** aus `apps/desktop/web/vendor/monaco/` via `python apps/desktop/web/vendor/fetch_monaco.py` – gitignored, versions-/hash-gepinnt, nie CDN – sonst Textarea; Speichern `PUT /workspace/{mission}/file` ⇒ `workspace.write` durch das Gateway; Diff aus `workspace.file.changed`, Terminal aus `workspace.run.output`, Agent Rail aus `agent.*`, Quality aus `workspace.run.finished` + `verification.*`, Artifacts aus `artifact.created`, Preview-iframe über `GET /workspace/{mission}/preview/<path>` mit CSP-Sandbox); Event-Batching pro Frame, kein Polling außer `/health` alle 5 s. Tauri-Wrapper folgt als eigener Milestone.
