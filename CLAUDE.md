...
- `core/` – neuer JARVIS Core (Phase 1 läuft): `core/events/` (Envelope, EventBus, SQLEventStore) und `core/missions/` (Mission/Task-Modelle, State Machine, MissionEngine, Repository) vorhanden; api, state, permissions, memory, models, agents, capabilities, verifier, scheduler folgen. Eigene Abhängigkeiten: `core/requirements.txt`.
...
- Kein Modell erhält direkten Zugriff auf OS, Dateien, Netzwerk oder Secrets.
