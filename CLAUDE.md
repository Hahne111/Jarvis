...
- `core/` – neuer JARVIS Core (Phase 1 läuft): `core/events/` (Envelope, EventBus, SQLEventStore) `core/missions/` (Mission/Task-Modelle, State Machine, MissionEngine, Repository) `core/permissions/` (P0–P6, Policy nur verschärfbar, Approval-Workflow, PermissionEngine) `core/capabilities/` (Manifest, Registry, Mocks `mock.echo`/`mock.clock`/`mock.open_url`, ExecutionGateway mit Timeout/Retry/Kill Switch) `core/verifier/` (Outcome, VerifierRegistry, VerificationService, RetryPolicy, VerifiedExecutor), `core/intents/` (deterministischer Fast-Path-Router), `core/api/` (FastAPI: /health, /events, /ws/events, /commands, /missions, /approvals, /kill, /resume, /devices, /missions/{id}/handover, /notifications, /home, /satellite/command, /news*, /telemetry, /schedule*, /suggestions*, /brief, /privacy, /skills*, /debug) `core/devices/` (Geräte-Registry mit Ed25519-Keypairs, Enrollment-Codes single-use, signierte Requests `X-Jarvis-Device|Timestamp|Nonce|Signature`, Nonce/Zeitfenster; `Caller`: Loopback = lokaler Owner, signiert ⇒ Trust aus Registry, remote unsigniert ⇒ nie trusted; starke Proofs nur gebunden; Revoke final; Forwarding-Header ⇒ kein Loopback-Owner; `python -m core enroll`; ADR-0004) `core/notify/` (PushService: `permission.ask`/`gateway.halted`/`mission.failed`/`device.revoked` → `FakePush`|`WebhookPush`, `JARVIS_PUSH=off|fake|webhook`, Events `notify.sent|failed`, `GET /notifications`) `core/news/` (World Intelligence Globe Datenseite: `NewsEvent` mit Land/Region/lat-lon/Topics/Quellen/Confidence/`breaking`/`forecast`, `geo.py` 68 Länder + Topic-Keywords deterministisch, `NewsStore`, `NewsPipeline` Dedupe/Cluster/Confidence mit Events `news.event.created|updated`, Quellen Fake/RSS-Atom über httpx+stdlib, `JARVIS_NEWS=off|fake|rss` + `JARVIS_NEWS_FEEDS`, Capabilities `news.refresh|top` P0, API `/news*`, `POST /telemetry`) `core/scheduler/` (durable `Job`s: Textkommando oder P0-Capability, `every_s`/`at`+Wochentage, Budget; `Scheduler.tick()` führt fällige Jobs über `run_text_command` aus ⇒ echte Missionen, Scheduler ist nie trusted; Watchdog ⇒ `mission.watchdog` + paused/failed; Loop via Lifespan, `JARVIS_SCHEDULER=on`) `core/proactive/` (`RelevanceEngine` ⇒ Kanal now/opportunistic/brief/silent, gated Push mit `notify.suppressed`; `HabitDetector` aus `command.received` ⇒ `automation.suggested`, Accept ⇒ Job + P0-Preload, nie automatisch; `brief.generate` P0 ⇒ `brief.ready`; `privacy.get|set` normal|private|guest verschärft Memory-Policy, pausiert Satelliten, Push nur critical; API `/schedule*`, `/suggestions*`, `/brief`, `/privacy`) `core/presence.py` (PresenceService: idle/listening/thinking/speaking/working/awaiting_approval/halted pro Gerät, deterministisch aus Events abgeleitet, `presence.changed`, `GET /presence`; HUD-Event-Vertrag in `docs/HUD_EVENTS.md`) `core/runtime.py` (CoreRuntime verdrahtet alles; `python -m core` startet auf 127.0.0.1:7870) `core/models/` (IntelligenceProvider-Interface, ModelRouter Fast/Smart/Deep, AgentBudget, MockProvider, ClaudeProvider – Anthropic SDK optional via `pip install anthropic`, Key nur über Umgebung/Credential-Broker) und `core/agents/` (AgentCoordinator: Router → Provider → Allowlist → VerifiedExecutor → Budget; pausiert bei Approval und setzt aus dem Event-Log fort; Subagent-Rollen research/implementation/test/verification/security über Tool `agent.delegate`, kontext-isoliert, Tiefe 1, gemeinsames Budget; Coding-Workflow: Rollen binden `workspace.*` (test nur `pytest`/`python` in `workspace.run`), Mission mit Codeänderung gilt erst nach verifiziertem grünem `workspace.run` als COMPLETED, sonst `not_verified`; `artifact.created` pro Workspace-Datei beim Abschluss; `JARVIS_PROVIDER=claude|mock|none`) und `core/memory/` (MemoryItem nach SPEC §8.2, MemoryStore mit lexikalischer Suche + austauschbarem Embedder, MemoryWriter: Privacy-Filter, Reinforcement, Korrektur als neue Version, Forget/Forget-Window/Pin/Temporary; ContextBuilder speist relevante, nicht-geheime Items in den Agent-Systemprompt; Capabilities `memory.recall|remember|correct|forget` über das Gateway; API `/memory*` = „What JARVIS Knows“; Events tragen nie den Wert) vorhanden; state und scheduler folgen. Eigene Abhängigkeiten: `core/requirements.txt`.
...
- `adapters/home/` – Home Core (Phase 8, SPEC §11): `HomeBackend`-Protokoll mit `FakeHome` (Tests/CI) und `HomeAssistantBackend` (REST `/api/states`, `/api/services`; `JARVIS_HA_URL`, Token nur aus `JARVIS_HA_TOKEN`, nie in Events/Logs/repr), `DeviceRegistry` (Entity → Raum → Etage, `resolve()` für entity_id/Raum/Name/„all“, mehrdeutig ⇒ Fehler), `HomeStateMachine` (Home/Away/Sleep/Work/Movie/Guests/Night/Vacation mit Policies, Rebuild aus `home.state.changed`). Capabilities `home.list_devices|get_state|state.get` P0, `home.light.set|switch.set|cover.set|climate.set|scene.activate|state.set` P2 (Verifier = Read-back), `home.lock.set|alarm.set|garage.set` P4 + trusted device ⇒ starker Proof, nie per Sprache (SECURITY §4). Aktivierung `JARVIS_HOME=off|fake|homeassistant`; `GET /home`; Fast-Path „licht an im wohnzimmer“, „turn on the kitchen light“, „szene movie“, „gute nacht“. `adapters/home/wol.py`: `power.status` P0, `power.wake` P3 (Magic Packet nur an Ziele aus `JARVIS_WOL_TARGETS`, Verifier `power.host_reachable` = TCP-Handshake, MAC nie in Events; `JARVIS_HOME=fake` ⇒ Demo-Ziel auf `FakeNetwork`). Voice-Satelliten: `POST /satellite/command` (HA Assist „Hey Jarvis“ → Core; Satellit untrusted per Default, Sprache erfüllt nie P3+, Antwort `speech`). `home.state.set apply_defaults=true` setzt Licht/Klima-Defaults nur für erlaubte Domains. HUD-Panel HOME. Anleitung: `docs/HOME_ASSISTANT.md`.
- `apps/desktop/web/` – web-first HUD-Shell (ADR-0003): reines HTML/CSS/ES-Module ohne Build, vom Core unter `http://127.0.0.1:7870/hud/` ausgeliefert (`/` leitet dorthin); zentraler AI-Core aus `presence.changed`, Event Rail (WS `after_seq`, Bereichsfilter), Approvals, Missions, Memory, Kill Switch, Latenz-Kacheln, Coding-Modus (Projektbaum, Editor = Monaco **nur lokal** aus `apps/desktop/web/vendor/monaco/` via `python apps/desktop/web/vendor/fetch_monaco.py` – gitignored, versions-/hash-gepinnt, nie CDN – sonst Textarea; Speichern `PUT /workspace/{mission}/file` ⇒ `workspace.write` durch das Gateway; Diff aus `workspace.file.changed`, Terminal aus `workspace.run.output`, Agent Rail aus `agent.*`, Quality aus `workspace.run.finished` + `verification.*`, Artifacts aus `artifact.created`, Preview-iframe über `GET /workspace/{mission}/preview/<path>` mit CSP-Sandbox); Event-Batching pro Frame, kein Polling außer `/health` alle 5 s. Mobile: responsive < 760 px, PWA (`manifest.webmanifest`), Browser-Gerätesignatur (WebCrypto Ed25519 in IndexedDB, „enroll this device“), Tab DEVICES; Remote-Setup in `docs/REMOTE.md`. Scene-Manager `body[data-mode]` (core/globe/coding/mission, Wechsel per Nav oder `command.received`-Event), `globe.js` = Canvas-2D-Globus ohne Bibliothek (Marker aus `/news/countries`, Country Cards + Evidence Rail, Quality Tiers aus Frame-Zeit, Telemetrie `globe_frame`/`hud_frame` an `POST /telemetry`). Tauri-Wrapper folgt als eigener Milestone.
...
python -m core                      # Provider: JARVIS_PROVIDER=claude (Default, braucht anthropic SDK + Key) | mock | none
                                    ...
                                    # Home:    JARVIS_HOME=off (Default) | fake | homeassistant (JARVIS_HA_URL, JARVIS_HA_TOKEN); WOL: JARVIS_WOL_TARGETS (siehe docs/HOME_ASSISTANT.md)
                                    # Remote:  JARVIS_CORE_HOST=<mesh-ip> (nie 0.0.0.0), JARVIS_PUSH=off|fake|webhook (docs/REMOTE.md); Enrollment-Code: python -m core enroll phone
                                    # Globe:   JARVIS_NEWS=off (Default) | fake | rss (JARVIS_NEWS_FEEDS=name=url@quality,...)
                                    # Proaktiv: JARVIS_SCHEDULER=on (Default im Prozess; Tests aus), JARVIS_HOME_COUNTRY=DE (News-Relevanz)
