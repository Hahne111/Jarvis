...
- `apps/desktop/web/` – web-first HUD-Shell (ADR-0003): reines HTML/CSS/ES-Module ohne Build, vom Core unter `http://127.0.0.1:7870/hud/` ausgeliefert (`/` leitet dorthin); zentraler AI-Core aus `presence.changed`, Event Rail (WS `after_seq`, Bereichsfilter), Approvals, Missions, Memory, Kill Switch, Latenz-Kacheln; Event-Batching pro Frame, kein Polling außer `/health` alle 5 s. Tauri-Wrapper folgt als eigener Milestone.
