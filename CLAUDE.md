...
- `core/` – neuer JARVIS Core (Phase 1 läuft): `core/events/` (Envelope, EventBus, SQLEventStore) `core/missions/` (Mission/Task-Modelle, State Machine, MissionEngine, Repository) und `core/permissions/` (P0–P6, Policy nur verschärfbar, Approval-Workflow, PermissionEngine) vorhanden; api, state, memory, models, agents, capabilities, verifier, scheduler folgen. Eigene Abhängigkeiten: `core/requirements.txt`.
